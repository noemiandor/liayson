...
aggregateSegmentExpression<-function(epg, segments, mingps, GRCh=37){
  ...
  epg=epg[x$ia,,drop=F];  genes=genes[x$ib,,drop=F]
  ...
  map=assignQuantityToMutation(epg,segments)
  iC=which(!is.na(map[,"quantityID"])); ##Genes that could be mapped to a segment
  s=rownames(segments)[map[iC,"quantityID"]]; ##Segments
  eps=.grpstats(epg[iC,cells],s,"mean")$mean; ##Expression per copy number segment
  
  ##Keep only "valid" segments, containing on average >=mingps expressed genes
  gps=.grpstats(epg[iC,cells]>0,s,"sum")$sum; ##Count genes expressed per copy number segment
  iK=which(apply(gps,1,median)>=mingps);
  eps=eps[iK,];  gps=gps[iK,];
