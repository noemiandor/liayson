.applyAR2seg<-function(varargin){
  ...
  for(cn_ in unique(out[!is.na(out)])){
    ...
    if(!isempty(eqI)) {   cn_status[ rules@rhs@data[eqI,] ]=cn_    }
  }
  
  
  ##Match each left hand rule to set of cells to which they apply
  cmap=matrix(F,length(rules),ncol(out)); colnames(cmap)=colnames(out)
  for(i in 1:length(rules)){
    seg1_EQ=rules@lhs@itemInfo[rules@lhs@data[,i],]
    cI=sapply(seg1_EQ,.testARstatement,out)
    cI=apply(cI,1,all)
    cI[is.na(cI)]=F
    cmap[i,names(cI)]=cI; ##Cells for which one can conclude copy number state of @seg
  }
  ...
  for(cell in colnames(out)){
    ...
    for(cn_ in unique(cn_status)){
      eq_ii=which(cmap[,cell] & cn_status==cn_) # All rules come to conclusion that seg has CNstate == ploidy
      if(!isempty(eq_ii)){
