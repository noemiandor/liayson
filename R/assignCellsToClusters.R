assignCellsToClusters<-function(outc, xps, similarity=T){
  fr=plyr::count(outc$sps); fr=fr[sort(fr$freq, decreasing = T, index.return=T)$ix,];     fr$x=as.character(fr$x)
  cnvs=t(.grpstats(t(outc$cnps),outc$sps,"mean")$mean)
  devFromInt=apply(abs(round(cnvs)-cnvs),1, median)
  seg=rownames(cnvs)[devFromInt<0.05]; ##Don't use segments with unstable CN states within given clone
  ...
  if(similarity){
    ...
    # diff=apply(d,2,max) - apply(d,2, function(x) sort(x,decreasing = T)[2])
    # sps[diff<1E-4]=NA
  }else{
    method="Euclidean"
  ...
  outc$cnps = outc$cnps[, x$ix];   outc$sps = x$x
  
  
  ##Visualize
  minV=quantile(as.numeric(d),0.025, na.rm=T);   d[d<minV]=minV; #Cap
  d=sweep(d, MARGIN = 2, apply(d,2,max), FUN = "/"); #Get distance matrix in shape
  
  HMCOLS=fliplr(brewer.pal(11,"RdBu")); 
  col=rainbow(max(outc$sps))
  tmp=outc$sps[names(outc$sps) %in% colnames(d)]
  hm = try(heatmap.2(t(d[,names(tmp)]), margins = c(13, 6), cexCol = 0.85, col = HMCOLS, colRow = col[tmp], 
                     Rowv = NULL, Colv = NULL,trace = "n", symm = F, hclustfun = function(x) hclust(x, method = "ward.D")))
  ...
  fr2=plyr::count(outc$sps[names(outc$sps) %in% colnames(xps)]); rownames(fr2)=as.character(fr2$x)
  y=fr2[fr$x,]$freq; y[is.na(y)]=0
